...
export const sortingCode = codePair(["mode: 'dataset',", "dataset: rows,", "orderByEnabled: true,", "columns: [", "  { name: 'name', label: 'Name' },", "  { name: 'area', label: 'Area', filterName: 'department', orderByEnabled: false, resizable: false },", "]", "// click a header for the sort menu; Shift+click adds a column to a multi-sort", "// (cycles asc → desc → removed). Apply it from code with a single OrderBy…", "controller.applyOrderBy({ name: 'name', direction: 'asc' })", "// …or a full multi-column order (index 0 sorts first):", "controller.applyOrderBy([", "  { name: 'area', direction: 'asc' },", "  { name: 'name', direction: 'desc' },", "])", "// or toggle one column additively (asc → desc → removed):", "controller.toggleOrderBy('name', { additive: true })"]);
